...
- **Change an entry's export signature** — when `preserveEntrySignatures: 'strict'`, adding modules to an entry chunk would expose symbols that the original entry didn't export. Only an export whose *canonical* symbol is owned by one of the merged modules counts: a chunk exports the symbols it declares, so a re-export barrel resolving its names to symbols living elsewhere is served by whichever chunk holds those owners, and folding the barrel in adds nothing.
