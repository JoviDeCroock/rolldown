...
Consumer-local eligibility never reads the order-wrap plan, and `apply_order_wraps` materializes a non-empty consumer-local plan even when that plan is empty (an all-interop graph, or an on-demand round with no at-risk modules): pre-chunk placement applied the same routes unconditionally, so skipping the lowering would emit the barrel's monolithic interop body against per-consumer leaf placement and cycle the chunks (#10515). The one exception is a single-entry build with code splitting disabled and an empty plan — there the probe's routes had no placement consumer, and an inert on-demand plan keeps its promise of flag-off-equivalent output (`cjs_reexport_barrel_code_splitting_disabled`). The disabled option alone is not the fact that matters: a plugin-emitted chunk still adds a second entry with per-entry placement (`disabled_splitting_emitted_entry` in the strict invariants suite), so the exception is keyed on entry count as well.
...
Both entry reachability and the acyclicity check use the static imports that will actually be emitted. `predicted_static_import_targets` combines the symbol-derived dependencies recorded by `patch_module_dependencies` (`symbol_derived_dependencies`), side-effectful direct imports, and the side-effectful runtime flag. It filters transitive side-effect dependencies propagated through a `sideEffects: false` barrel because those do not become imports of the barrel's chunk; retaining them would invent cycles (`optimization/chunk_merging/already_loaded_side_effectful_barrel`).

Entry reachability follows this graph from the entry's own atom. Under `strictExecutionOrder`, the pass keeps using `dependencies` because order lowering can turn linked import records back into `init_*` imports.

When the reduced bitset would put an atom into a single purely dynamic-imported entry chunk, the reduction is not restricted by `preserveEntrySignatures`. If every dynamic importer only reads a statically-known subset of exports from the namespace (`DynamicImportExportsUsage::Partial`), the extra exports are unobservable and the atom is merged as-is, with no extra intermediate namespace. Otherwise, since we control all dynamic importers of the chunk, we generate a synthetic namespace inside the chunk, and use it as the "imported namespace" in the importer code (`import("./chunk.js").then((n) => n.<ns>)`). We bail out in case of a `then` export, or of a dynamic/external `export * from`. The rewrite also requires bundler-controlled export naming, so it is disabled under `preserveModules` and when `minifyInternalExports` is off.
...
Used indirect re-exports need an additional execution edge when package metadata declares the re-exporter itself side-effect-free but retains observable work in its body. `patch_module_dependencies` records those symbol-specific edges in `indirect_reexport_load_dependencies`, separately from the module-wide `load_dependencies` union. The ordinary reachability walk places the retained re-exporter body for each consumer; cross-chunk linking later emits a bare import for those recorded edges when the target is unwrapped. Wrapped strict-order targets instead rely on import-record-derived `init_*` obligations; multi-hop symbol-only routes under wrap-all strict execution are a known obligation gap. Keeping the provenance separate avoids treating unrelated symbol-owner or entry-placement dependencies as side effects, and direct `export ... from` records remain transparent.
...
- **Change an entry's export signature** — when `preserveEntrySignatures: 'strict'`, adding modules to an entry chunk would expose symbols that the original entry didn't export. Only an export whose _canonical_ symbol is owned by one of the merged modules counts: a chunk exports the symbols it declares, so a re-export barrel resolving its names to symbols living elsewhere is served by whichever chunk holds those owners, and folding the barrel in adds nothing.
...
The cycle check reads the temporary graph as it stands, so a candidate rejected for a cycle can become mergeable once _other_ merges collapse the chunks that formed that cycle. Merging therefore runs in rounds until a whole round makes no progress; only then are the surviving candidates materialized as common chunks, in their original order so chunk creation stays independent of how many rounds it took. A staircase of shared modules with descending dependent-entry sets needs this: the widest candidate is blocked by the narrower ones that are themselves about to be folded into the same entry (`optimization/chunk_merging/common_chunk_merge_unblocked_by_other_merges`, issue #9963). The entry-signature decision, by contrast, is fixed up front — it does not depend on which merges already happened.
